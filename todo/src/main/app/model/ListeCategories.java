...
public class ListeCategories implements Serializable {
    ...
    public ListeCategories() {
        setCategories(new Vector<>());
        // Création de 3 catégories par défaut ("sans", "travail","personnel")
        Categorie c1 = new Categorie("Sans");
        Categorie c2 = new Categorie("Travail");
        Categorie c3 = new Categorie("Personnel");
    ...
    public void choixCategorie(int ind, int type, String titre, Date echeance, Date debut, int importance) {
    ...
    public void modifTache(Tache t, int ind, int p) {
        int indice = 0;
        boolean bool = false;
        int i = 0;
        while (!bool && i < getCategories().size()) { // On recherche à quelle catégorie appartient la tâche passée en paramètre.
            if (Objects.equals(getCategories().elementAt(i).getTitre(), t.getCategorie())) {
                bool = true;
                indice = i;
            }
            i++;
    ...
    private void setCategories(Vector<Categorie> categories) {
